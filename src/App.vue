...
const generateFood = () => {
    ...
    while (foods.value.length < targetCount) {
        ...
        if (boardCache.value[idx] === 'EMPTY' && !isOccupiedBySnake && !isInFrogSafeZone(newFood.x, newFood.y)) {
...
const generateBombs = () => {
    ...
    while (bombs.value.length < targetCount) {
        ...
        if (boardCache.value[idx] === 'EMPTY' && !isOccupiedBySnake && !isInFrogSafeZone(newBomb.x, newBomb.y)) {
...
const handleKeyUp = (e) => {
    if (gameState.value !== 'PLAYING') return;
    if (e.key === 'ArrowUp' && nextDirection.value.dir === 'UP' ||
...
const triggerGameOver = () => { stopGame(); gameState.value = 'GAME_OVER'; };
const triggerVictory = () => { stopGame(); gameState.value = 'VICTORY'; };
    ...
    <div class="game-container">
        <div class="game-board neon-black-board" :class="{ 'neon-black-board-boosting': isBoosting }">
            ...
            >
              <div v-if="index === 0" 
                   class="frog-head" 
                   :class="'frog-dir-' + s.dir.toLowerCase()">
                🐸
              </div>
              <div v-else 
...
@keyframes score-box-pulse {
  0%, 100% { box-shadow: 0 0 12px rgba(57, 255, 20, 0.2); border-color: rgba(57, 255, 20, 0.7); }
...
.neon-black-board-boosting {
  ...
  box-shadow: 0 0 40px rgba(57, 255, 20, 0.5), inset 0 0 20px rgba(57, 255, 20, 0.15);
}
...
.game-over-title { color: #ff5252; font-family: 'Orbitron', sans-serif; font-size: calc(var(--custom-cell-size) * 1.4); text-shadow: 0 0 10px #ff5252; }
...
.neon-btn-green:active { transform: scale(0.96); }
...
.neon-btn-orange:active { transform: scale(0.96); }
...
.neon-btn-white:active { transform: scale(0.96); }
...
.neon-btn-gold:active { transform: scale(0.96); }
...
.pause-hint-text { margin-top: 25px; font-size: calc(var(--custom-cell-size) * 0.42); color: #666; font-family: 'Share Tech Mono', monospace; }
...
.modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.75); display: flex; justify-content: center; align-items: center; z-index: 999; backdrop-filter: blur(4px); }
.modal-content { background-color: #151518; border: 2px solid #2e7d32; padding: 35px 40px; border-radius: 12px; width: 700px; max-width: 90%; box-shadow: 0 0 30px rgba(46, 125, 50, 0.3); text-align: left; }
.modal-content h2 { margin-top: 0; color: #39ff14; font-family: 'Orbitron', sans-serif; font-size: 1.8rem; text-align: center; text-shadow: 0 0 8px rgba(57,255,20,0.5); }
.modal-content hr { border: 0; border-top: 2px solid #222; margin: 20px 0; }
.rules-list { padding-left: 20px; margin-bottom: 25px; }
.rules-list li { margin-bottom: 16px; font-size: 1.05rem; line-height: 1.6; color: #cccccc; }
.rules-list strong { color: #39ff14; }
.close-btn { display: block; width: 100%; padding: 12px 0; background-color: #2e7d32; color: white; border: none; border-radius: 6px; font-size: 1.1rem; font-weight: bold; cursor: pointer; transition: background-color 0.15s ease; font-family: 'Space Grotesk', sans-serif; letter-spacing: 1px; }
.close-btn:hover { background-color: #39ff14; color: #000; box-shadow: 0 0 15px #39ff14; }
